...
int main(int argc, char **argv) {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "BEHOLD", nullptr, nullptr);

    VkSurfaceKHR vkSurface = nullptr;
    auto surfRes = glfwCreateWindowSurface(vkbInstance.instance, 
                                            window, nullptr, &vkSurface);
    ...
    auto graphIndex = vkbDevice.get_queue_index(vkb::QueueType::graphics).value();
    auto presentIndex = vkbDevice.get_queue_index(vkb::QueueType::present).value();
    ...
    auto vertSrc = readBinaryFile(
        "build/compiledshaders/Exercises03/shader.vert.spv"
    );
    auto fragSrc = readBinaryFile(
        "build/compiledshaders/Exercises03/shader.frag.spv"
    );
    ...
    for(int i = 0; framebuffers.size(); i++) {
